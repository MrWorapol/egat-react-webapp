...
} from "../../../state/summary-report/settlement-report/settlement-detail-state";
import {
  IImbalanceReport,
  ITradeContractReport,
  settlementReportState,
} from "../../../state/summary-report/settlement-report/settlement-report-state";
...
export function useSettlementReport() {
  ...
  const resetSettlementReportState = useResetRecoilState(settlementReportState);
  const resetSettlementDetailState = useResetRecoilState(settlementDetailState);
  const resetSettlementChartState = useResetRecoilState(settlementChartState);
  
  const settlementAPI = new SettlementReportAPI();
  const userMeterApi = new UserAndEnergyReportAPI();


  const refreshSettlementReport = async () => {
    if (session) {
      try {
        showLoading(10);
        const userMeterInfos = await userMeterApi.getUserMeterInfo({
          period,
          session,
        });
        const req = {
        ...
        const tradeContractReports = await settlementAPI.getTradeContractReport(
          req
        );
        const tradeDataReport = await settlementAPI.getTradeDataReport(req);
        let summaryRole: ISummaryMap = {
          aggregator: 0,
          prosumer: 0,
          consumer: 0,
        };
        ...
        let summaryImbalanceType: ISummaryMap = {
          energyExcess: 0,
          energyShortfall: 0,
        };
        ...
        if (
          tradeContractReports &&
          userMeterInfos &&
          tradeDataReport &&
          tradeContractReports.context.length > 0 &&
          userMeterInfos.context.length > 0 &&
          tradeDataReport.context.length > 0
        ) {
          let output: ITradeContractReport[] = [];
          tradeContractReports.context.forEach(
            (contract: ITradeContractReport) => {
              //map tractContract
              let buyerMeter = userMeterInfos.context.find(
                (user: IUserMeterInfo) => {
                  return user.id.toString() === contract.buyerId.toString();
                }
              );
              let sellerMeter = userMeterInfos.context.find(
                (user: IUserMeterInfo) => {
                  return user.id.toString() === contract.sellerId.toString();
                }
              );
              contract.imbalance = []; //create array variable for insert imbalance Data
              let tradeDataMapContractId: IImbalanceReport[] =
                tradeDataReport.context.filter(
                  (imbalance: IImbalanceReport) => {
                    return (
                      imbalance.tradeContractIds.toString() ===
              ...
              if (tradeDataMapContractId.length > 0) {
                if (
                  tradeDataMapContractId.every((tradeData) => {
                    return (
                      tradeData.tradeType === "SELLER_CONTRACT" ||
                      tradeData.tradeType === "BUYER_CONTRACT"
                    );
                  })
                ) {
                  //case CONTRACT Not use
                } else {
                  let imbalanceCase = tradeDataMapContractId.filter(
                    (tradeData) => {
                      return (
                        tradeData.tradeType !== "SELLER_CONTRACT" &&
                        tradeData.tradeType !== "BUYER_CONTRACT"
                    ...
                    imbalanceCase.forEach((imbalance) => {
                      switch (imbalance.tradeType) {
                        ...
                        case "SELLER_IMBALANCE_OVERCOMMIT":
                          ...
                          summaryNet["netSale"] +=
                            contract.priceCommitted +
                            contract.tradingFee +
                            contract.wheelingChargeFee;
                          summaryNet["netImbalance"] += imbalance.price;
                          summaryImbalnceAmount["sellerOverCommit"] +=
                            imbalance.amount;
                          break;
                        case "BUYER_IMBALANCE_OVERCOMMIT":
                          ...
                          summaryNet["netBuy"] +=
                            ...
                            contract.wheelingChargeFee;
                          summaryNet["netImbalance"] += imbalance.price;
                          summaryImbalnceAmount["buyerOverCommit"] +=
                            imbalance.amount;
                          break;
                        //case under commit
                        case "SELLER_IMBALANCE_UNDERCOMMIT":
                          ...
                          summaryNet["netSale"] +=
                            ...
                            contract.wheelingChargeFee;
                          summaryNet["netImbalance"] += imbalance.price;
                          summaryImbalnceAmount["sellerUnderCommit"] +=
                            imbalance.amount;
                          break;
                        case "BUYER_IMBALANCE_UNDERCOMMIT":
                          ...
                          summaryNet["netBuy"] +=
                            ...
                            contract.wheelingChargeFee;
                          summaryNet["netImbalance"] += imbalance.price;
                          summaryImbalnceAmount["buyerUnderCommit"] +=
                            imbalance.amount;
                          break;
                        default:
                          break;
  ...
  const refreshSettlementDetail = async (settlement: ITradeContractReport) => {
    ...
    let newSettlementDetail: ISettlementDetail = {
      ...
      netPrice:
        settlement.priceCommitted +
        settlement.tradingFee +
        settlement.wheelingChargeFee, // NET Buy | NET sale w/o imbalance price
    ...
    if (settlement.imbalance && settlement.imbalance.length === 0) {
      //case CONTRACT
      newSettlementDetail.energyDeliverd = settlement.energyCommitted; // energy deliverd = committed
      newSettlementDetail.netEnergyPrice =
        newSettlementDetail.netPrice / settlement.energyCommitted; // = Price / energyPerKWh
    } else {
      // CASE IMBALANCE
      switch (newSettlementDetail.userType) {
        case "BUYER":
          ...
          let imbalanceBuyer =
            ...
            settlement.imbalance.find((tradeData: IImbalanceReport) => {
              return (
                settlement.buyerId === tradeData.buyerId ||
                settlement.buyerId === tradeData.sellerId
              );
            });
          if (
            imbalanceBuyer &&
            settlement.imbalanceStatus === "energyShortfall"
          ) {
            newSettlementDetail.energyDeliverd =
              settlement.energyCommitted - imbalanceBuyer.amount;
            newSettlementDetail.orderImbalanceAmount = imbalanceBuyer.amount;
            newSettlementDetail.orderImbalance = imbalanceBuyer.price;
            newSettlementDetail.netEnergyPrice =
              newSettlementDetail.energyDeliverd === 0
                ? 0
                : newSettlementDetail.netPrice /
                  newSettlementDetail.energyDeliverd;
          }
          // }
          if (imbalanceBuyer && settlement.imbalanceStatus === "energyExcess") {
            newSettlementDetail.energyDeliverd = imbalanceBuyer.amount;
            newSettlementDetail.orderImbalanceAmount =
              imbalanceBuyer.amount > settlement.energyCommitted
                ? imbalanceBuyer.amount - settlement.energyCommitted
                : imbalanceBuyer.amount;
            newSettlementDetail.orderImbalance = imbalanceBuyer.price;
            /*ไม่มั่นใจเพราะไม่มีเคสของ excess ตัวอย่าง*/ newSettlementDetail.netEnergyPrice =
              newSettlementDetail.energyDeliverd === 0
                ? 0
                : newSettlementDetail.netPrice /
                  newSettlementDetail.energyDeliverd;
        ...
        case "SELLER":
          let imbalanceSeller =
            settlement.imbalance &&
            settlement.imbalance.find((tradeData) => {
              return (
                settlement.sellerId === tradeData.buyerId ||
                settlement.sellerId === tradeData.sellerId
              );
            });
          if (
            imbalanceSeller &&
            ...
            newSettlementDetail.energyDeliverd =
              settlement.energyCommitted - imbalanceSeller.amount;
          ...
          if (
            imbalanceSeller &&
            settlement.imbalanceStatus === "energyExcess"
          ) {
            newSettlementDetail.energyDeliverd = imbalanceSeller.amount;
            newSettlementDetail.orderImbalanceAmount =
              imbalanceSeller.amount > settlement.energyCommitted
                ? imbalanceSeller.amount - settlement.energyCommitted
                : imbalanceSeller.amount;
            newSettlementDetail.orderImbalance = imbalanceSeller.price;
            /*ไม่มั่นใจเพราะไม่มีเคสของ excess ตัวอย่าง*/ newSettlementDetail.netEnergyPrice =
              newSettlementDetail.energyDeliverd === 0
                ? 0
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      resetSettlementReportState();
      resetSettlementDetailState();
      resetSettlementChartState();
    };
  }, []);
